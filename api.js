...
const API = {
  ...
  init() {
    if (!localStorage.getItem(STORAGE_KEYS.FOODS) && typeof INITIAL_DATA !== "undefined") {
      ...
      // Auto-migrate old male placeholder avatar to girl student avatar if present in existing storage
      const girlAvatarUrl = "https://images.unsplash.com/photo-1494790108377-be9c29b29330?auto=format&fit=crop&w=200&q=80";
      const curUser = Utils.getStorage(STORAGE_KEYS.CURRENT_USER);
      if (curUser && curUser.avatar && curUser.avatar.includes("photo-1535713875002-d1d0cf377fde")) {
        curUser.avatar = girlAvatarUrl;
        if (curUser.name === "Aarav Sharma" || curUser.name === "Aditya Verma") curUser.name = "Aanya Sharma";
        Utils.setStorage(STORAGE_KEYS.CURRENT_USER, curUser);
      }

      const allUsers = Utils.getStorage(STORAGE_KEYS.USERS, []);
      let changed = false;
      allUsers.forEach(u => {
        if (u.avatar && u.avatar.includes("photo-1535713875002-d1d0cf377fde")) {
          u.avatar = girlAvatarUrl;
          if (u.name === "Aarav Sharma" || u.name === "Aditya Verma") u.name = "Aanya Sharma";
          changed = true;
        }
      });
      if (changed) Utils.setStorage(STORAGE_KEYS.USERS, allUsers);
    }
  },
