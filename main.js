...
const App = {
  ...
  renderUserNavState() {
    ...
    navAuthContainers.forEach((container) => {
      ...
      if (user) {
        const hasAvatar = Boolean(user.avatar && user.avatar.trim() !== "" && user.avatar !== "none");
        const avatarHtml = hasAvatar
          ? `<img src="${user.avatar}" alt="${Utils.escapeHtml(user.name)}" class="user-nav-avatar">`
          : `<span class="user-nav-avatar-icon" title="No profile picture"><i class="fa-solid fa-user"></i></span>`;

        container.innerHTML = `
          <div class="user-dropdown-wrapper">
            <button class="user-nav-btn" id="user-menu-trigger" aria-haspopup="true">
              ${avatarHtml}
              <span class="user-nav-name">${(user.name || "Student").split(" ")[0]}</span>
              <span class="user-nav-badge">₹${Number(user.walletBalance || 0).toFixed(0)}</span>
